...
ggplot(world_clim,aes(fill=tmean))+
  geom_sf()+
...
world_clim%>%
  group_by(continent)%>%
  top_n(n=1,wt=tmean)%>%
  dplyr::select(name_long,continent,tmean)%>%
  arrange(desc(tmean))%>%
...
l=world_clim%>%
  leaflet() %>%
        ...
        addPolygons(stroke = FALSE, smoothFactor = 0.2, fillOpacity = 0.5,
                    color = pal(world_clim$tmean),
                    popup = paste("Mean Annual Temperature = ", round(world_clim$tmean, 2)," C"))%>%
  addLegend("bottomright", pal = pal, values = ~tmean,
