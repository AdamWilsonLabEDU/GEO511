...
knitr::include_graphics("PS_01_img/data-science.png")
```
...
If you want to use models for inference (e.g. hypothesis testing):
...
You can't use the same data to generate a hypothesis and test it! 

## Data use in model building and testing

Hypothesis testing: one approach is to split your data into three pieces before you begin the analysis:

1.  60% of your data: __training__ (or exploration) set. Do anything you like: visualise it and fit tons 
    of models to it.
  
1.  20% __query__ set. Use these data to compare models 
    or visualisations, but don't use it as part of an automated process.
...
1.  __Model family__ express a precise, but generic, pattern that you want to capture. For example, the pattern 
...
1.  __Fitted model__: find the model from the family that is the closest to your data. This takes the generic model 
    family and makes it specific, like `y = 3 * x + 7` or `y = 9 * x ^ 2`.

## Model fitting

A fitted model is just the closest model from a family of models. You may have the "best" model (according to some criteria) but it still may not be 'a good model' and does not mean that the model is "true". George Box puts this well in his famous aphorism:

> All models are wrong, but some are useful.


## Model fitting - George Box
...
> question of interest is _"Is the model illuminating and useful?"_.
...
Let's consider a linear model, i.e. `y = a_0 + a_1 * x`.  
...
There are 250 models on this plot, but a lot are really bad! How do we select the good ones?
