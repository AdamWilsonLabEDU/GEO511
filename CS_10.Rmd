...
```{r, fig.height=6, warning=F, purl=F}
# plot it
gplot(lulc)+
...
lst_month=tapp(lst,index = "months",fun = mean,na.rm=T)
...
monthly=global(lst_month,mean,na.rm=T) %>% round(2) %>% data.frame()
...
lctl=gather(lct.summary, key="month", value="value", -var, -Land_Cover_Type_1_13)

lctl$month=factor(lctl$month,
                   labels=month.name,
                   ordered=T)
lctl$Land_Cover_Type_1_13=lcd$landcover[as.numeric(lctl$Land_Cover_Type_1_13)+1]
...
## Build summary table
dplyr::filter(lctl,count>=50,
    Land_Cover_Type_1_13%in%c("Urban & built-up","Deciduous Broadleaf forest")) %>% 
  mutate(txt=paste0(round(mean,2),
                    "  (±",round(sd,2),")"))%>%
  dplyr::select(Land_Cover_Type_1_13,txt,month)%>%
  spread(Land_Cover_Type_1_13,txt)%>%
